...
#
# 1x1 degree GFS
#$URL='http://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.2016013006/gfs.t06z.pgrb2.1p00.f012'
$URL='http://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.$YYYY$MM$DD$HH/gfs.t${HH}z.pgrb2.1p00.f${FHR3}';
#$URL='http://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.2016013006/gfs.t06z.pgrb2.1p00.f0000.idx'
#$URL='http://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.2016013006/gfs.t06z.pgrb2.1p00.f000.idx'
...
while ($fhr <= $hr1) {
   ...
   if ($range eq '') {
      $err = system("$curl -f -v -r \"$range\" $url$grb -o $OUTDIR/$file.tmp");
      $err = $err >> 8;
      if ($err != 0) {
      ...
      print "no matches (no download) for $url$file\n";
   }
   $fhr += $dhr;
}
print "\n\nfinished download\n\n$output\n";
